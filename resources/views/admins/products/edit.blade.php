...
@section('adm-title', 'Edit Products')
@section('contents')
...
<form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="mb-5">
    ...
    <div class="row mb-3">
        <div class="col">
            ...
            <textarea name="descriptions" class="form-control @error('descriptions') is-invalid @enderror" rows="10">{{ $product->descriptions }}</textarea>
        </div>
        @error('descriptions')
            <span class="invalid-feedback">{{ $message }}</span>
        @enderror
    ...
    <div class="row mb-3">
        ...
        <div class="col">
            ...
            <select name="new_stock" class="form-select @error('new_stock') is-invalid @enderror">
                <option value="">--select--</option>
                <option value="1" @selected($product->new_stock == 1)>Yes</option>
                <option value="0" @selected($product->new_stock == 0)>No</option>
            </select>
            @error('new_stock')
                <span class="invalid-feedback">{{ $message }}</span>
            @enderror
    ...
    <div class="row mb-3">
        <div class="col">
            ...
            <select name="in_stock" class="form-select @error('in_stock') is-invalid @enderror">
                <option value="">--select--</option>
                <option value="1" @selected($product->in_stock == 1)>Yes</option>
                <option value="0" @selected($product->in_stock == 0)>No</option>
            </select>
            @error('in_stock')
                <span class="invalid-feedback">{{ $message }}</span>
            @enderror
